...
/opt/nifi/nifi-toolkit-current/bin/tls-toolkit.sh client -c {{ .Values.ca.server }} -t {{ .Values.ca.token }} --dn "CN=$(hostname -f), OU=${OU:-BIGDATA}" -p 9443
...
echo "192.0.14.142 zk-zookeeper-0.zk-zookeeper-headless.tkg-bigdata-ns.svc.cluster.local" >> /etc/hosts
echo "192.0.26.61 zk-zookeeper-1.zk-zookeeper-headless.tkg-bigdata-ns.svc.cluster.local" >> /etc/hosts
echo "192.0.8.69 zk-zookeeper-2.zk-zookeeper-headless.tkg-bigdata-ns.svc.cluster.local" >> /etc/hosts
